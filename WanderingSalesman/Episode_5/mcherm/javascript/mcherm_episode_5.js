...
function countPaths(startNode, length) {
    ...
    //     powerMatrix = matrixMultiply(powerMatrix, adjacencyMatrix);
    // }
    ...
    if (length > 1) {
        ...
        while(true) { // will reduce digitNum each time through and exit when digitNum === 0
            ...
            if (binaryDigit === "1") {
                resultMatrix = matrixMultiply(resultMatrix, powerOfTwoMatrix);
            }
            ...
            digitNum -= 1;
            powerOfTwoMatrix = matrixMultiply(powerOfTwoMatrix, powerOfTwoMatrix);
        }
    }
